...
4. `POST /v1/events/ack` records acknowledgement and advances `subscriptions.cursor` to the acked event id when that event is newer. Failed webhook deliveries retry independently per hook (5 attempts, exponential backoff) so a down endpoint cannot stall other hooks.
